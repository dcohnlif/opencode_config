---
description: Summarize today's OpenCode activity across tracked projects
---

# Daily Summary

Generate a concise bulleted summary of what I worked on **today**, by querying the OpenCode session database.
...
Query sessions that were **active today** — use `time_updated` (not `time_created`) so that multi-day sessions are included. Use `strftime('%s','now','start of day') * 1000` as the epoch-ms threshold for midnight today (UTC). Exclude subagent sessions (`parent_id IS NOT NULL`).
...
If no sessions are found, output: **"No activity found for today."** and stop.
...
- **Deduplicate** — if multiple sessions cover the same topic (e.g., continuing the same task across days), merge them into one bullet
...
- **Substance** — focus on *what was accomplished*, not process details like "explored code" or "ran queries". If a session was purely exploratory Q&A with no concrete outcome, summarize the topic investigated.
